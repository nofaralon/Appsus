...
function getGoogleBooks(val = `effectivejavascript`) {
    var googleBooks = utilService.loadFromStorage(GOOGLEB_KEY) || {}
    if (googleBooks && googleBooks[val]) return Promise.resolve(googleBooks[val])
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${val}`)
        .then(res => {
            console.log('getting data from server');
            googleBooks[val] = res
            utilService.saveToStorage(GOOGLEB_KEY, googleBooks)
            console.log('res', res);
            return res
        })
        .catch(err => {
